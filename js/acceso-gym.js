...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function groupAccessesByMonth(accesses) {
    ...
    accesses.forEach((access) => {
      // Usar la fecha original sin ajuste
  ...
  function countMonthlyAttendanceDays(monthlyAccesses) {
    ...
    for (const [monthYear, accesses] of Object.entries(monthlyAccesses)) {
      ...
      accesses.forEach((access) => {
        // Usar la fecha ajustada si está disponible, o la fecha original sin ajuste
        const date = access.adjustedDate || new Date(access.timestamp)
  ...
  accessForm.addEventListener("submit", (e) => {
    ...
    if (!isActive) {
      showResult(
        `<i class="fas fa-exclamation-triangle"></i> ¡Acceso registrado! Tu membresía ha vencido (pasaron ${daysSinceRegistration} días desde tu inscripción). Por favor, renueva tu plan.`,
        "warning",
      )
    } else {
      if (remainingDays <= 5) {
        showResult(
          `<i class="fas fa-exclamation-triangle"></i> ¡Acceso registrado! Tu membresía vence en ${remainingDays} día${remainingDays !== 1 ? "s" : ""}.`,
          "warning",
        )
      } else {
        showResult(
          `<i class="fas fa-check-circle"></i> ¡Acceso registrado correctamente! Te quedan ${remainingDays} días de membresía.`,
          "success",
        )
      }
    }
